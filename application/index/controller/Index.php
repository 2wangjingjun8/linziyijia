...
class Index extends controller{
    ...
    public function weixinset(){
        $options = array(
            ...
            'appid'=>'wx71ba0a52ebf23ab9', //填写高级调用功能的app id
            'appsecret'=>'147614c1ef3ccd38c1319912543823ef' //填写高级调用功能的密钥
        );
    ...
    private function push($wechat, $type){
        $replyData = Db::name("jbadminddata")->field('ShopAddress,Phone,AboutUs,KeyWord,BeConcerned,CardId,OtherMsg')->find();
        ...
        switch ($type) {
            case Wechat::MSGTYPE_TEXT:
                ...
                $keywords = explode(';',$replyData['KeyWord']);
                $flag = 1;
                foreach ($keywords as $k => $v) {
                    $keywordArr = explode(':',$v);
                    if($keywordArr[0] == $data['Content']){
                        $flag = 0;
                        $wechat->text($keywordArr[1])->reply();
                    }
                }
                if($flag){
                    $wechat->text($replyData['OtherMsg'])->reply();
                }
                break;
            case Wechat::MSGTYPE_EVENT:
                switch ($data['Event']) {
                    case Wechat::EVENT_SUBSCRIBE:
                        $wechat->text($replyData['BeConcerned'])->reply();
                        break;
                    ...
                    case Wechat::EVENT_MENU_CLICK:
                        ...
                        if($key=='ConsumeList'){
                            ...
                            $str = $replyData['ShopAddress'];
                            $wechat->text($str)->reply();
                        }else if($key=='phone'){
                            $str = $replyData['Phone'];
                            $wechat->text($str)->reply();
                        }else if($key=='AboutUs'){
                            $str = $replyData['AboutUs'];
                            $wechat->text($str)->reply();
                        }else if($key=='CardTicket'){
                            $card_id = $replyData['CardId'];
                            $url = $wechat->createCardQrcode($card_id);
                            $qrurl = $wechat->getQRUrl($url['ticket']);
                            // dump($url);exit;
                            $str = "<a href='".$qrurl."'>点我，扫码，领取卡券</a>";
                            $wechat->text($str)->reply();
                            $wechat->getQRUrl($url['ticket'])->reply();
                        }
                        break;
    ...
    public function cztpl($chongzhi){
        ...
        $TempId = Db::name("jbadminddata")->field('RechargeTempId')->find();

        $data =[];
        $data['touser'] = $openid;
        $data['template_id'] = $TempId['RechargeTempId'];
        $data['url'] = '';
        $data['topcolor'] = '#FF0000';

        $msg = [ 
            'first' => ['value' => '您好，您已经充值成功。', 'color' => '#ff0000'], 
            ...
            'remark' => ['value' => '感谢您对我们的支持，我们将为您提供更优质的服务。', 'color' => '#ff0000']
        ];
        $data['data'] = $msg;
    ...
    public function xftpl($order){
        ...
        $openid = $this->getUserOpenid($cardcode);
        if(empty($openid)){
            Db::name('qtsaleorder')->where('CardCode',$cardcode)->update(['IsSend'=>2]);
            file_put_contents(ROOT_PATH .'/xftpl.txt', "\r\n".date('Y-m-d H:i:s').' addcardno='.input('addcardno').' openid 为空' ,FILE_APPEND);
            return false;
        }
        ...
        $TempId = Db::name("jbadminddata")->field('ConsumeTempId')->find();

        $data =[];
        $data['touser'] = $openid;
        $data['template_id'] = $TempId['ConsumeTempId'];
        $data['url'] = 'http://' . $_SERVER['HTTP_HOST'] . url('Index/jilu', array('CardsCode' => $cardcode));
        $data['topcolor'] = '#FF0000';
        $youhui = $order['Amount']-$order['RealAmount'];
        $msg = [ 
            // 'first' => ['value' => '', 'color' => '#ff0000'], 
            'keyword1' => ['value' => $order['OrderDate'], 'color' => '#000000'], 
            'keyword2' => ['value' => $order['ShopNAme'], 'color' => '#000000'], 
            'keyword3' => ['value' => $order['RealAmount'], 'color' => '#000000'], 
            'keyword4' => ['value' => $youhui, 'color' => '#000000'], 
            'keyword5' => ['value' => $order['OrderNo'], 'color' => '#000000'], 
